...
def build_processor(base_model, vocab_path):
    ...
    if vocab_path.exists():
        ...
        print(f"WARN: vocab not found at {vocab_path}, falling back to base tokenizer")
...
def make_training_args(args, use_fp16, use_bf16):
    """Build TrainingArguments — handle both old (evaluation_strategy) and new (eval_strategy) APIs"""
    common = dict(
    ...
    # Try new API first (transformers >= 4.46)
    try:
...
def main():
    ...
    if args.skip_augmented:
        ...
        if len(ds["train"]) == 0:
            sys.exit("ERROR: train set empty after filtering!\n"
                     "  Run without --skip_augmented OR use build_manifest.py instead")
    ...
    # Prepare features (load audio via librosa, no torchcodec needed)
    def prepare(example):
